...
@objc public class AsyncDownloader : NSObject {
    ...
    public class var sharedInstance: AsyncDownloader {
        ...
        dispatch_once(&Static.onceToken) {
            Static.instance = AsyncDownloader()
        }
    ...
    override init() {
        ...
        if(!AsyncDownloader.isSetConfig){
    ...
    public func fetchJsonURL(  config : RequestConfig , withCompletion completion:( reponse  : JSON  )->Void ){
        ...
        if let link = config.link {
            ...
            NSException(name: "AsyncDownloader", reason: "config.link cannot be nil", userInfo: nil).raise()
        }
    }
    ...
    public func loadFile(  config : RequestConfig , withCompletion completion:( fileData  : NSData  )->Void ){
        ...
        if (config.link != nil && config.imageView != nil) {
            ...
            NSException(name: "AsyncDownloader", reason: "( config.link || config.imageView ) cannot be nil", userInfo: nil).raise()
        }
    }
